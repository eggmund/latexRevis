...
Feynman diagrams are a way of visualising particle interactions.

They show space in one axis, and time in the other.

\begin{center}
\includegraphics[width=0.8\textwidth]{images/betaMinus.png}
\end{center}

Here is an example using $\beta^-$ decay. Time goes along the $y$ axis, and space goes along the $x$ axis.

Charge is conserved at junctions. In this example because the neutron is turning into a proton ($+1$), the $W$ boson has to be negatively charged to conserve charge.
...
Here is a Feynman diagram for electron capture:

\begin{center}
\includegraphics[width=0.8\textwidth]{images/electronCaptureFeyn.png}
\end{center}
...
When two antiparticles collide/interact, they annihilate each other, converting the mass of both of the particles into energy in the form of two photons. \textbf{Two photons are produced to conserve momentum.}
...
Free vibrations is when there is no driving force. There is no transfer of energy to or from the surroundings (is a closed system).

If you leave a mass spring system to oscillate without a driver, this is still called free vibrations even though there is damping (air resistance).

\subsubsection{Forced Vibrations}

Forced vibrations happen when there is an external driving force. The system is forced to vibrate by the driver. At low frequencies, the oscillator follows the motion of the driver (with a large amplitude), so the two are in phase. If the driver has a frequency much higher than the resonant frequency then the oscillator can't keep up, so the amplitude is very small. The driver is completely out of phase (180\textdegree) with the oscillator.

\subsubsection{Resonance}

Resonance happens when the driving frequency is the same as the resonant frequency (when the oscillator oscillates 90\textdegree out of phase).

\begin{center}
\includegraphics[width=0.5\textwidth]{images/resonantFrequency.png}
\end{center}
